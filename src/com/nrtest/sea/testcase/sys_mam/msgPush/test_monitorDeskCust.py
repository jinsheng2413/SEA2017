...
# 系统管理→信息定制→监控台定制
@ddt
class TestMonitorDeskCust(unittest.TestCase, MonitorDeskCustPage):
    @classmethod
    def setUpClass(cls):
        print('开始执行')
        # 打开菜单（需要传入对应的菜单编号）
        cls.driver = openMenu(MsgPushData.MonitorDeskCust_para)
    ...
    def query(self, para):
        ...
        sleep(2)
        # 校验
        result = self.assert_context(*MonitorDeskCustLocators.CHECK_FIRST)
