...
class RegularSporadicUpgradeLocators:
    ...
    # 升级版本号
    UPGRADE_VERSION_NO = (
        By.XPATH, '//label[contains(text(),"升级版本号")]/../div/div/img')
    # 升级版本号→值
    UPGRADE_VERSION_NO_VALUE = (
        By.XPATH, '(//div[@class="x-combo-list-inner"])[4]/div[%s]')
    # 起始终端地址
    TMNL_ADDR_START = (
        By.XPATH, '//label[contains(text(),"起始终端地址")]/../div/input')
    # 结束终端地址
    TMNL_ADDR_END = (
        By.XPATH, '//label[contains(text(),"结束终端地址")]/../div/input')
    # 终端资产号
    TMNL_ASSET_NO = (
