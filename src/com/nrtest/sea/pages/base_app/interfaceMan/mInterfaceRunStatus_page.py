...
class MInterfaceRunStatusPage(Page):
    ...
    def inputSel_business_system(self, index):
        ...
        locator = self.get_select_locator(MInterfaceRunStatusLocators.QRY_BUSINESS_SYSTEM_VALUE, index)
        self.click(*locator)

    # 服务对象名称
    def inputSel_service_name(self, index):
        self.click(*MInterfaceRunStatusLocators.QRY_SERVICE_NAME)
        locator = self.get_select_locator(MInterfaceRunStatusLocators.QRY_SERVICE_NAME_VALUE, index)
        self.click(*locator)
        print('------')
