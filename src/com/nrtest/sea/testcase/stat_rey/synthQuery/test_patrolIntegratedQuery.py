...
@ddt
class TestPatrolIntegratedQuery(TestCase, PatrolIntegratedQueryPage):
    @classmethod
    def setUpClass(cls):
        # 打开菜单（需要传入对应的菜单编号）ljf
        menuPage = MenuPage.openMenu(SynthQuery_data.PatrolIntegratedQuery_para)
        super(TestCase, cls).__init__(cls, menuPage.driver, menuPage)
        # 菜单页面没多个Tab页时，请注释clickTabPage所在行代码
        menuPage.clickTabPage(SynthQuery_data.PatrolIntegratedQuery_tabName)
    ...
    @BeautifulReport.add_test_img()
    @data(*DataAccess.getCaseData(SynthQuery_data.PatrolIntegratedQuery_para,
                                  SynthQuery_data.PatrolIntegratedQuery_tabName, valCheck=True))
