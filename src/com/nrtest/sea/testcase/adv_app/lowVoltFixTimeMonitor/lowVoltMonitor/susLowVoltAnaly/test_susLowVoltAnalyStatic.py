...
@file: test_susLowVoltAnalyStatic.py
...
@ddt
class test_SusLowVoltAnaly(TestCase, SusLowVoltAnalyStaticPage):
    ...
    def setUpClass(cls):
        ...
        menuPage.clickTabPage(LowVoltFixTimeMonitorData.para_SusLowVoltAnaly_static)
...
from com.nrtest.sea.data.adv_app.lowVoltFixTimeMonitor.lowVoltFixTimeMonitor_data import py


# 高级应用→低压固定时间点电压电流监测→低电压监测→疑似低电压分析:疑似低电压用户统计
@ddt
class test_SusLowVoltAnaly(TestCase, SusLowVoltAnalyPage):

    @classmethod
    def setUpClass(cls):
        # 打开菜单（需要传入对应的菜单编号）
        menuPage = MenuPage.openMenu(py.susLowVoltAnaly_para)
        super(TestCase, cls).__init__(cls, menuPage.driver, menuPage)
        # 菜单页面没多个Tab页时，请注释clickTabPage所在行代码
        menuPage.clickTabPage(py.susLowVoltAnaly_SusLowVoltAnalyStatic)
        # 菜单页面上如果没日期型的查询条件时，请注释下面代码
        menuPage.remove_dt_readonly()
    ...
    @BeautifulReport.add_test_img()
    @data(*DataAccess.getCaseData(py.susLowVoltAnaly_para, py.susLowVoltAnaly_SusLowVoltAnalyStatic))
    def test_query(self, para):
        """高级应用→低压固定时间点电压电流监测→低电压监测→疑似低电压分析:疑似低电压用户统计
        """
        self.start_case(para, __file__)
    ...
    @BeautifulReport.add_test_img()
    @data(*DataAccess.getCaseData(py.susLowVoltAnaly_para, py.susLowVoltAnaly_SusLowVoltAnalyStatic, valCheck=True))
    def _test_checkValue(self, para):
        self.start_case(para, __file__)
        self.query(para)
        self.assert_query_criteria(para)
