...
@ddt
class TestDistributedEnergyLoadMonitor(unittest.TestCase, DistributedEnergyLoadMonitorPage):
    @classmethod
    def setUpClass(cls):
        print('开始执行')
        # 打开菜单（需要传入对应的菜单编号）
    ...
    @data(
        *DataAccess.getCaseData(DistributedEnergyMange_data.DistributedEnergyLoadMonitor_para, tabName='分布式电源负荷监测统计'))
