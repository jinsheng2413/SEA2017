...
# 系统管理→系统配置管理→IP黑名单管理
@ddt
class TestIpBlacklistMan(TestCase, IpBlacklistManPage):
    @classmethod
    def setUpClass(cls):
        # # 打开菜单（需要传入对应的菜单编号）
        menuPage = MenuPage.openMenu(SysConfigManData.IpBlacklistMan_para)
    ...
    @BeautifulReport.add_test_img()
    @data(*DataAccess.getCaseData(SysConfigManData.IpBlacklistMan_para, valCheck=True))
