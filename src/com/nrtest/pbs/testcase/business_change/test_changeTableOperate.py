...
@file: test_changeTableOperate.py
@time: 2019-03-07 10:04
@desc:
"""
...
class TestChangeTableOperate(TestCase, ChangeTableOperatePage):
    ...
    def setUpClass(cls):
        # 打开菜单（需要传入对应的菜单编号）
        menuPage = MenuPage.openMenu(BusinessChange_data.ChangeTableOperate_para)
    ...
    def query(self, para):
        ...
        # 资产号
        self.inputStr_asset_no(para['ASSET_NO'])
        # 查询按钮
        self.btn_qry()
    ...
    @BeautifulReport.add_test_img()
    @data(*DataAccess.getCaseData(BusinessChange_data.ChangeTableOperate_para,
                                  BusinessChange_data.ChangeTableOperate_tabName_operate, valCheck=True))
