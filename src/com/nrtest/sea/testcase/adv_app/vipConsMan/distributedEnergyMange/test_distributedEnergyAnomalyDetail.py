...
@ddt
class TestDistributedEnergyAnomalyDetail(unittest.TestCase, DistributedEnergyAnomalyDetailPage):
    @classmethod
    def setUpClass(cls):
        print('开始执行')
        # 打开菜单（需要传入对应的菜单编号）
        cls.driver = openMenu(
            DistributedEnergyMange_data.DistributedEnergyAnomalyAnalysis_para, True)
