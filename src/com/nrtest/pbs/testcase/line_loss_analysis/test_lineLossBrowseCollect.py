...
@file: test_lineLossBrowseCollect.py
@time: 2019-03-14 15:27
@desc:
"""
...
# 线损分析→线损浏览:线损汇总
@ddt
class TestLineLossBrowseCollect(TestCase, LineLossBrowseCollectPage):
    @classmethod
    def setUpClass(cls):
        # 打开菜单（需要传入对应的菜单编号）
    ...
    @data(*DataAccess.getCaseData(LineLossAnalysis_data.lineLossBrowse_para,
                                  LineLossAnalysis_data.lineLossBrowse_tab_collect))
    ...
    @BeautifulReport.add_test_img()
    @data(*DataAccess.getCaseData(LineLossAnalysis_data.lineLossBrowse_para,
                                  LineLossAnalysis_data.lineLossBrowse_tab_collect, valCheck=True))
