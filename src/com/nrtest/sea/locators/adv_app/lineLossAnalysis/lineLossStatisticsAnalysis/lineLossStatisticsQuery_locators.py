...
class LineLossStatisticsQueryLocators:
    # 线损分类
    QRY_LINE_LOSS_TYPE = (By.XPATH, '//label[contains(text(),"线损分类")]/../div/div/img')
    QRY_LINE_LOSS_TYPE_VALUE = (By.XPATH, '//div[@class="x-combo-list-inner"]/div[%s]')
    # 线损率
    QRY_LINE_LOSS_RATE = (By.XPATH, '//label[contains(text(),"线损率")]/../div/div/img')
    QRY_LINE_LOSS_RATE_VALUE = (By.XPATH, '(//div[@class="x-combo-list-inner"])[2]/div[%s]')
    QRY_LINE_LOSS_RATE_INPUT = (By.XPATH, '//input[@class=" x-form-text x-form-field x-form-num-field "]')
    # 日期
