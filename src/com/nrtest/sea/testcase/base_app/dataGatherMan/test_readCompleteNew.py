...
from com.nrtest.sea.pages.base_app.dataGatherMan.readCompleteNew_page import ReadCompleteRatePageNew
from com.nrtest.sea.pages.other.menu_page import MenuPage


# 基本应用→数据采集管理→采集质量检查(new)→采集完整率
@ddt
class TestReadCompleteRate(TestCase, ReadCompleteRatePageNew):
    ...
    def query(self, para):
        ...
        if tab_name == '采集完整率':
            ...
            # 查询
            self.btn_qry()
        elif tab_name == '采集完整率统计' or tab_name == '采集完整率明细':
            # 用户类型
            self.inputSel_cons_type(para['CONS_TYPE'])
    ...
    @data(*DataAccess.getCaseData(DataGatherMan_data.dataGatherQualityAnalyze_new_para,
                                  DataGatherMan_data.dataGatherQualityAnalyze_new_readComplete))
