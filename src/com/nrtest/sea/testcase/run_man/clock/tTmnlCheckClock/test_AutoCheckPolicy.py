...
# 自动对时策略配置
@ddt
class TestAutoCheckPolicy(unittest.TestCase, AutoCheckPolicyPage):
    ...
    def setUpClass(cls):
        ...
        # 点击Tab页标签
        clickTabPage(ClockData.para_TTmnlCheckClock_policy)
    ...
    @data(*DataAccess.getCaseData(ClockData.para_TTmnlCheckClock,
                                  ClockData.para_TTmnlCheckClock_policy))
    ...
    @BeautifulReport.add_test_img()
    @data(*DataAccess.getCaseData(ClockData.para_TTmnlCheckClock,
                                  ClockData.para_TTmnlCheckClock_policy, valCheck=True))
