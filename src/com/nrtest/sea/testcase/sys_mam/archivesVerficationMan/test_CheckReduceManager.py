...
class TestCheckReduceManager(TestCase, CheckReduceManagerPage):
    ...
    def setUpClass(cls):
        # 打开菜单（需要传入对应的菜单编号）
        menuPage = MenuPage.openMenu(ArchivesVerficationMan_data.checkReduceManager_para)
    ...
    @BeautifulReport.add_test_img()
    @data(*DataAccess.getCaseData(ArchivesVerficationMan_data.checkReduceManager_para, valCheck=True))
