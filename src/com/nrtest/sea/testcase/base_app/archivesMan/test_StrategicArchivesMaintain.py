...
class TestStrategicArchivesMaintain(TestCase, StrategicArchivesMaintainPage):
    ...
    def setUpClass(cls):
        # 打开菜单（需要传入对应的菜单编号）
        menuPage = MenuPage.openMenu(ArchivesMan_data.strategicArchivesMaintain_para)
    ...
    @BeautifulReport.add_test_img()
    @data(*DataAccess.getCaseData(ArchivesMan_data.strategicArchivesMaintain_para, valCheck=True))
