...
@file: test_changeLossBrowseCollect.py
@time: 2019-03-14 16:43
@desc:
"""
...
# 线损分析→变损浏览:变损汇总
@ddt
class TestChangeLossBrowseCollect(TestCase, ChangeLossBrowseCollectPage):
    @classmethod
    def setUpClass(cls):
        # 打开菜单（需要传入对应的菜单编号）
    ...
    @data(*DataAccess.getCaseData(LineLossAnalysis_data.changeLossBrowse_para,
                                  LineLossAnalysis_data.changeLossBrowse_tab_collect))
    ...
    @BeautifulReport.add_test_img()
    @data(*DataAccess.getCaseData(LineLossAnalysis_data.changeLossBrowse_para,
                                  LineLossAnalysis_data.changeLossBrowse_tab_collect, valCheck=True))
