...
class TestSysOperationLog(unittest.TestCase, SysOperationLogPage):
    ...
    def setUpClass(cls):
        print('开始执行')
        # 打开菜单（需要传入对应的菜单编号）
        cls.driver = openMenu(LogEdit_data.sysOperationLog_para)
    ...
    def query(self, para):
        ...
        sleep(2)
        # 校验
        result = self.assert_context(*SysOperationLogLocators.CHECK_FIRST)
        self.assertTrue(result)

    @BeautifulReport.add_test_img()
    @data(
        *DataAccess.getCaseData(LogEdit_data.sysOperationLog_para, LogEdit_data.sysOperationLog_tabName_tmnl))
