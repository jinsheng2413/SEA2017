...
# 运行管理→时钟管理→本地广播校时设置
@ddt
class TestDemo(unittest.TestCase, RemoteBroadChkClockPage):

    @classmethod
    def setUpClass(cls):
        print("开始执行")
        # 打开菜单（需要传入对应的菜单编号,Ture的作用：利用中文名称点击菜单）
        cls.driver = openMenu(ClockData.para_RemoteBroadChkClock, True)
        sleep(2)
    ...
    def query(self, para):
        ...
        self.sleep_time(2)
        # 校验
        result = self.assert_context(*RemoteBroadChkClockLocators.TABLE_DATA)
