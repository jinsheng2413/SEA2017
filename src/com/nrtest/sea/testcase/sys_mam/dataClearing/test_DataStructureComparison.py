...
from com.nrtest.sea.pages.sys_mam.dataClearing.dataStructureComparison_page import DataStructureComparisonPage,DataStructureComparisonLocators
...
# 系统管理-->数据清理管理-->数模结构对照
@ddt
class TestDataStructureComparison(unittest.TestCase,DataStructureComparisonPage):

    @classmethod
    def setUpClass(cls):
        print("开始执行")
        # 打开菜单（需要传入对应的菜单编号）
        cls.driver = openMenu(DataClearing_data.dataStructureComparison_para)
        sleep(2)
        cls.exec_script(cls,DataStructureComparisonLocators.START_DATE_JS)
