...
@file: test_SettlementPointDayAbnormal.py
...
@ddt
# 智能研判-->结算点日异常
class Test_settlementpointdayabnormal(TestCase, SettlementPointDayAbnormal_page):

    @classmethod
    def setUpClass(cls):
        # 打开菜单（需要传入对应的菜单编号）
        menuPage = MenuPage.openMenu(SmartTo_data.SettlementPointDayAbnormal_para)
    ...
    @BeautifulReport.add_test_img()
    @data(*DataAccess.getCaseData(SmartTo_data.SettlementPointDayAbnormal_para, valCheck=True))
